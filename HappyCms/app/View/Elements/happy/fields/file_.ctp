...
echo $this->Form->input($name,array('type'=>'hidden','id'=>'input-file-'.$name.'-'.$lang));

?>
<div class="input file">
<label><?php echo $options['label']; ?></label>
<div id="file-<?php echo $name.'-'.$lang ?>">
<div  class="a pickfiles"><?php echo $this->Html->image('/img/boxupload32.png'); ?><a id="file-<?php echo $name.'-'.$lang ?>-pickfiles" >A partir de mon ordinateur</a></div>
<div  class="a distantfiled"><?php echo $this->Html->image('/img/Slideshow.png'); ?><a  id="file-<?php echo $name.'-'.$lang ?>-distantfiled" >Images existantes</a></div>
<div class="image"></div>
...
<?php if(empty($this->request->data['Content']['id']))
{
    $this->request->data['Content']['id'] = null ;
} ?>
...
$(function() {
    ...
    ?>];
    var fileName = <?php 
    $modelName = $this->request->data['Happy']['model_name'];
    echo empty($this->request->data[$modelName][$lang][$name])?'null':"'".$this->request->data[$modelName][$lang][$name]."'"; ?>;
    var file = new fileField(<?php echo "'".$ExtensionName."','".$name."','".$lang."','".$this->request->data[$modelName]['id']."'"; ?>,fileName,filters);
