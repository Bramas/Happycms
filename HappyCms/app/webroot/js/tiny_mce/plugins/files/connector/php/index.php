...
class TinyImageManager {
	...
	function TinyImageManager() {
		...
		$this->ALLOWED_FILES = array('jpeg','jpg','gif','png','doc','docx','ppt','pptx','xls','xlsx','mdb','accdb', 'swf', 'zip', 'rar', 'rtf', 'pdf', 'psd', 'mp3', 'wma');
	...
	function DirStructure($type, $innerDirs='first', $currentDir='', $level=0) {
		...
		foreach ($innerDirs as $v) {
			...
			if(isset($v['childs'])) {
				...
				if($soc>0 && end($currentDirArr)==$v['name']){
					$folderAct = 'folderAct';
				} else {
					$folderAct = '';
				}
	...
	function ShowDir($dir, $type) {
		...
		foreach ($dir as $v) {
			...
			if($v['general']['width'] > WIDTH_TO_LINK || $v['general']['height'] > HEIGHT_TO_LINK) {
				...
				$middle_thumb_attr = 'fmiddle="'.ABSOLUTE_ROOT.$middle_thumb.'" fmiddlewidth="'.$middle_width.'" fmiddleheight="'.$middle_height.'" fclass="'.CLASS_LINK.'" frel="'.REL_LINK.'"';
			} else {
				$middle_thumb = '';
				$middle_thumb_attr = '';
			}
			...
			$ret .= '
   <table class="imageBlock0" cellpadding="0" cellspacing="0" filename="'.$v['general']['filename'].'" fname="'.$v['general']['name'].'" type="'.$type.'" ext="'.strtoupper($v['general']['ext']).'" path="'.ABSOLUTE_ROOT.$v['general']['path'].
   '" linkto="'.ABSOLUTE_ROOT.$v['general']['link'].'" fsize="'.$v['general']['size'].'" fsizetext="'.$this->bytes_to_str($v['general']['size']).'" date="'.date('d.m.Y H:i',$v['general']['date']).'" fwidth="'.$v['general']['width'].'" fheight="'.$v['general']['height'].
   '" md5="'.$v['general']['md5'].'" '.$middle_thumb_attr.'><tr><td valign="bottom" align="center">
    <div class="imageBlock1">
     <div class="imageImage">
      <img src="'.ABSOLUTE_ROOT.$thumb.'" width="100" height="100" alt="'.$v['general']['name'].'" />
     </div>
	...
	function GetThumb($dir, $md5, $filename, $mode, $width=100, $height=100) {
		...
		if ($isfile)
		{
			$server_url = rtrim(dirname($_SERVER["SCRIPT_FILENAME"]), '/').'/../../';
			$server_url = realpath($server_url);
			$server_url = str_replace('\\','/',rtrim($server_url, '/').'/img/fileicons/');
			$url=DIR_PLUGIN.'/img/fileicons/';
