...
var ImagesDialog = {
	...
	insert : function(text,url) {
		
		
		var win = tinyMCEPopup.getWindowArg("window");
		
		if(win != undefined && win.document.getElementById(tinyMCEPopup.getWindowArg("input")))
		{
...
$(function(){
	...
	$.ajax({
		type: "POST",
		url: "connector/php/",
		data: "action=showpath&pathtype=files&type=files&path=&default=1",
	...
	$.ajax({
		type: "POST",
		url: "connector/php/",
		data: "action=showtree&pathtype=files&type=files&default=1",
		success: function(data){
			$('#tree').html(data);
		}
	});
	...
	$.ajax({
		type: "POST",
		url: "connector/php/",
		data: "action=showdir&pathtype=files&type=files&path=&default=1",
		success: function(data){
			$('#loader').hide();
			//$('#files').html(data);
	...
	$('.imageBlock0').live('dblclick', function(){
		...
		ImagesDialog.insert(text,e.attr('linkto'));
