<?php
define ("MAXMEM", 30*1024*1024);  // 30M
//ini_set('memory_limit','30M');
$width=intval($_GET['width']);
$height=intval($_GET['height']);
$img = 'media/'.$_GET['folder'].'/'.$_GET['filename'].'.'.$_GET['ext'];
$dest = 'thumbs/'.$_GET['folder'].'/'.$_GET['filename'].'_'.$width.'x'.$height.'.'.$_GET['ext'];
...
class Image{
	...
	**/
	public static function resize($img,$dest,$largeur=0,$hauteur=0){
		...
		if($dimension[0] * $dimension[1] * 6 > MAXMEM )
		{
			return false;
		}
		...
		//si la largeur demandé ou la hauteur demandé dépasse les vrai valeur on conserve l'image d'origine
		if($largeur>=$dimension[0] || $hauteur>=$dimension[1])
		{
			copy($img, $dest);
			return;
		}
