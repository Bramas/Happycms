...
class ContentsController extends AppController
{
    ...
    {
        $this->data = $this->Content->item(1,$params,true);
        $this->data['Menu']['can_appear'] = $menu_id;
    ...
    function admin_files($extension,$filter=null)
    {
        ...
        if ($handle = opendir('../webroot/files/uploads/'.$extension.'/')) {    
            while (false !== ($file = readdir($handle))) {
                ...
                {
                    echo '<div class="item">'.
                    $html->image($html->url('/files/uploads/'.$extension.'/'.$file,true),array('height'=>60)).
    ...
    function admin_item_edit($extension,$item_id=null)
    {
      ...
      //  $this->Extension->load($extension);
        ...
        //debug(
            ...
            $this->data['Happy']['force_all_languages']=$controller->Hforce_all_languages;
...
die('{"jsonrpc" : "2.0", "result" : null, "id" : "id","realName":"'.$fileName.'"}');
	...
	App::import('Controller',$extension);
	$controllerName = $extension.'Controller';
	$controller = new $controllerName();
	$prefix = empty($this->params['prefix'])?'':$this->params['prefix'].'_';
    ...
    {
        $controller->data= $this->params['named']['data'];
    }
    
    if(!empty($this->params['named']['lang_form']))
    {
        $controller->_requestedLanguage=$this->params['named']['lang_form'];
    ...
    if(method_exists($controller, $controller->action))
    {
       $output = call_user_func_array(array(&$controller,$controller->action),$controller->passedArgs);
        ...
        $controller->data['Happy'] = $this->params['named'];
        ...
        $controller->data['Happy']['Extension']=$itemExtension;
        $controller->data['Happy']['model_name'] = empty($this->params['named']['model_name'])?$controller->Hmodel_name:$this->params['named']['model_name'];
        
	    if(isset($this->params['named']['data']))
        {
            $controller->data= array_merge((array)$this->params['named']['data'],(array)$controller->data);
        }
        
        $controller->data['Happy']['force_all_languages']=$controller->Hforce_all_languages;
        
    	if ($controller->autoRender) {
    		$controller->output = $controller->render(null,'happy/controller_edit');
    ...
    if(empty($controller->data['Happy']['output']))
    {
        $controller->data['Happy']['output']=array();
    }
    ...
    if(empty($view->data['Happy']['output']))
    {
        $view->data['Happy']['output']=array();
            ...
            return array_merge((array)$view->data['Happy']['output'],
                               array('output'=>$controller->output)
                               )
            ;
	//    $this->data = $this->requestAction('/admin/home/index_edit/12');
    ...
    function search()
    {
        if(empty($this->data['Content']['input']))
        {
            $this->redirect('/');
            exit();
        }
        $searchTerm = htmlentities($this->data['Content']['input']);
        $results = $this->Content->find('all',array('conditions'=>array("MATCH(params) AGAINST ('".utf8_decode(mysql_real_escape_string(htmlentities($this->data['Content']['input'], ENT_NOQUOTES, "UTF-8")))."')")));
       // debug(htmlentities($this->data['Content']['input'], ENT_NOQUOTES, "UTF-8"));
        ///exit();
        //debug($results);
        $searchResults=array();
        ...
        {
            App::import('Controller',ucfirst($result['Extension']['controller']));
            ...
            if(class_exists($controllerName))
            {
                ...
                {
                    if( ( $t = $controller->searchResult($result) ) !== false)
                    {
                        $t['content']=strip_tags($t['content']);
                        preg_match('/'.$this->data['Content']['input'].'/',$t['content'], $matches, PREG_OFFSET_CAPTURE);
                        ...
                        $t['content'] = substr($t['content'], max($matches[0][1]-100,0),strlen($this->data['Content']['input'])-max($matches[0][1]-100,0)+$matches[0][1]+100);

                        //debug(array(
                        //    max($matches[0][1]-100,0),
                        //strlen($this->data['Content']['input'])-max($matches[0][1]-100,0)+$matches[0][1]+100));

                        $t['content'] = '...'.preg_replace('/('.$this->data['Content']['input'].')/','<span class="ResultTermMatch">$1</span>',$t['content']).'...';
