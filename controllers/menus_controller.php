...
class MenusController extends AppController
{
   ...
   function admin_index() {
		$this->data = $this->Menu->generatetreelist(null, null, null, '|_');
		debug ($this->data); die;       
	}
    ...
    {
    	$this->data = $this->data = $this->getItem($this->data['Menu']['item_id']);
    ...
    function admin_to_trash($item_id,$lang=null)
    {
    	...
    	$ajax = !empty($this->data['ajax']);
	parent::admin_to_trash($item_id,$lang);
    ...
    function admin_add_new($parent_id,$move)
    {
	    ...
	    $this->data['Menu']['item_id']=$item_id;
	    ...
	    if($move>0)
	    {
			for($i=0;$i<$move;$i++)
			{
			     $this->Menu->moveup();//$this->data['Menu']['id']);
			}
	    }
	    if($move<0)
	    {
			for($i=0;$i<-$move;$i++)
			{
			     $this->Menu->movedown();//$this->data['Menu']['id']);
			}
		}
		$this->redirect(array(
			'controller'=>'menus',
	...
	if(!$this->data) { echo '{"result":false}'; exit(); }
	...
	if(empty($this->data['Menu']['id']))
	{
	    ...
	    $this->Menu->id=null;
	    $this->Menu->save(array('Menu'=>array('parent_id'=>$this->data['Menu']['parent_id'],'item_id'=>$item_id)));
	   
	    $this->Menu->recover();
	    
	    if($this->data['Move']>0)
	    {
		for($i=0;$i<$this->data['Move'];$i++)
		{
		     $this->Menu->moveup();//$this->data['Menu']['id']);
		}
	    }
    ...
    function admin_affect_module()
    {
		...
		list($ext,$view) = explode(':',$this->data['Menu']['extension'],2);
		$this->data['Menu']['extension']=$ext;
		$this->data['Menu']['view']=$view;
	
        $this->Menu->save($this->data);
        $this->data = $this->Menu->findById($this->Menu->id);
	
		$viewParams = $this->requestAction('/admin/'.$this->data['Menu']['extension'].
			     '/'.$this->data['Menu']['view'].'_new'.'/'.
			      $this->data['Menu']['id']);
		if(!empty($viewParams))
		{
			$this->Menu->save(array('Menu'=>array('id'=>$this->data['Menu']['id'],
                                                              'params'=>$viewParams)));
		}
	
        $this->redirect(array('controller'=>'contents','action'=>'item_edit','menus', $this->data['Menu']['item_id']));
        exit();
    }
	...
	if($id)
        {
	    $menu = $this->Menu->findById($id);
	    $this->data = $this->getItem($menu['Menu']['item_id']);
        }
    ...
    {
	$menu = $this->Menu->find('first',array('conditions'=>array('Menu.item_id'=>$this->data[$this->Hmodel_name]['id'])));
    ...
    function options_list($parent_id='IS NULL')
    {
       	//$this->Menu->Behaviors->attach('Content');
    ...
    {
    	if(empty($this->data['_Menu']['language']) || empty($this->data['_Menu']['id']))
    	{
    		
    		exit('empty data');
    	}
    	$this->_requestedLanguage = $this->data['_Menu']['language'];
    	$item = $this->getItem($this->data['_Menu']['id']);
