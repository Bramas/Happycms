...
class CategoriesController extends AppController
{
    ...
    function admin_save()
    {
        ...
        $this->request->data['Category']['item_id']= $content['Content']['item_id'];
        
        if(empty($this->request->data['Content']['id']))
        {
            $r = $this->Category->save(array('Category'=>$this->request->data['Category']
                    ));
            $id=$this->Category->id;
        }
        else{
            $r = $this->Category->find('first',array('conditions'=>array('Category.item_id'=>$this->request->data['Content']['id'])
                    ));
            $id=$r['Category']['id'];
        }
        $ret = '';
        foreach($this->request->data['Content'] as $lang=>$data)
        {
            ...
            $ret.=',"parent_id":'.$this->request->data['Category']['parent_id'];
        echo '{"newCat":{'.$ret.'}}';
        exit();
    }
