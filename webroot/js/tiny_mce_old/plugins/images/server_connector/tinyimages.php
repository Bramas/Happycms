...
class tinyimages 
{
	...
	function ajaxDelFile($input) 
	{
		...
		foreach ($input['src'] as $key=>$val) 
		{
			...
			if(is_file(DIR.$this->mydir.$uri))
			{
				unlink(DIR.$this->mydir.$uri);
			} 
			else 
			{	
				if(is_file(DIR.$this->mydir.$uri) && $key != '$family')
				{
					$error[] = 'Fichier '.DIR.$this->mydir.$uri.' non trouve! '.DIR.$this->mydir.$uri;
				}
			}
		}
	...
	function ajaxMakeFolder($input) 
	{
		...
		if(mkdir(DIR.$this->folder.'/'.$input['name']))
		{
			return array(); 
		}
		else 
