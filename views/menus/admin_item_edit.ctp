...
<span title="Cliquez pour <?php
 ...
 echo empty($this->data['_Menu']['published'])?'publier':'dépublier';

 ?> cette page" lang="<?php echo $HpLangForm; ?>" menu_id="<?php echo $this->data["_Menu"]['id']; ?>" class="togglePublished qtip <?php echo empty($this->data['_Menu']['published'])?'unpublished':'published'; ?>" 
    ...
    $(function(){
        /*$('#lang-tab-<?php echo $HpLangForm; ?> .togglePublished').click(function(){
            ...
            $.ajax({
                ...
                url:'<?php echo $html->url('/admin/menus/togglePublished') ; ?>',
                data:{
                    "data[ajax]":1,
                    "data[_Menu][id]":'<?php echo $this->data["_Menu"]['id']; ?>',
...
<span class="page-title"><?php echo (empty($this->data['_Menu']['title'])?'[title]':$this->data['_Menu']['title']); ?></span>
<div class="form hidden">
<?php

    echo $form->input('title', array('label'=>'Titre : ','class'=>'ui-corner-all input-title'));
...
<div class="other-input">
    ...
    <?php echo $form->input('published',array('type'=>'checkbox','label'=>false)); ?>
    </div>
    <?php echo $form->input('class',array('type'=>empty($is_linksite)?'hidden':'text')); ?>
</div>

<?php

echo $form->submit('^', array('class'=>'close','type'=>'button'));
?>
<div class="clear"> </div>
</div>
